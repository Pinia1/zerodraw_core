...
export class FrontendToolBridge {
  ...
  async wait(
    ...
    return new Promise((resolve, reject) => {
      ...
      getAgentLogger().info('[Agent FrontendTool] waiting', { sessionId, toolCallId, toolName });
    });
  }
  ...
  async complete(
    ...
    if (row.status !== 'pending') {
      getAgentLogger().info('[Agent FrontendTool] duplicate complete ignored', {
        sessionId,
    ...
    if (!waiting) {
      getAgentLogger().error('[Agent FrontendTool] complete without live waiter (process restarted?)', undefined, {
        sessionId,
        toolCallId: input.toolCallId,
    ...
    getAgentLogger().info('[Agent FrontendTool] complete', {
      sessionId,
      toolCallId: input.toolCallId,
