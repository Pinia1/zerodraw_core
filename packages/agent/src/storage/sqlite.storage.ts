...
import { getAgentSqlite, type AgentSqliteDatabase } from '../config';
import { addUsage } from '../usage/utils';

type Executor = AgentSqliteDatabase;
type ExecResult = { affectedRows: number };
type Row = Record<string, unknown>;

/**
 * SQLite 实现的 pi-agent `Storage`（better-sqlite3）。
 *
 * 严格对齐官方 sqlite backend（storageVersion 1, 7 张表）的持久化语义。
 * 每个 `SqliteStorage` 实例绑定一个 sessionId（由 repo 创建/打开时注入）。
 */
...
function query<T extends Row>(ex: Executor, sql: string, params: unknown[] = []): T[] {
  return ex.prepare(sql).all(...params) as T[];
}

function exec(ex: Executor, sql: string, params: unknown[] = []): ExecResult {
  return { affectedRows: ex.prepare(sql).run(...params).changes };
}

const dbQuery = <T extends Row>(sql: string, params: unknown[] = []) => query<T>(getAgentSqlite(), sql, params);
...
async function readNextSeq(ex: Executor, sessionId: string): Promise<number> {
  const rows = await query<Row>(ex, `SELECT next_seq FROM agent_sessions WHERE id = ?`, [sessionId]);
...
async function advanceNextSeq(ex: Executor, sessionId: string, nextSeq: number): Promise<void> {
  const result = await exec(
    ex,
    `UPDATE agent_sessions SET next_seq = ?, updated_at = ? WHERE id = ?`,
    [nextSeq, Date.now(), sessionId],
  );
...
async function insertBranchEntry(conn: Executor, sessionId: string, branchId: string, entry: Entry): Promise<void> {
  exec(conn,
...
async function createRootBranchForEntry(conn: Executor, sessionId: string, entry: Entry): Promise<void> {
  exec(conn,
...
async function appendEntryToExistingBranch(conn: Executor, sessionId: string, branchId: string, entry: Entry): Promise<void> {
  ...
  if (result.affectedRows !== 1) throw new Error(`Expected to update branch ${branchId}, updated ${result.affectedRows}`);
}

async function appendEntryToBranchIndex(conn: Executor, sessionId: string, entry: Entry): Promise<void> {
...
async function createDivergentBranchForEntry(conn: Executor, sessionId: string, entry: Entry): Promise<void> {
...
async function scanBranchSegments(sessionId: string, query: StorageBranchScan, readSegment: (segment: BranchSegment, oldestFirst: boolean, stopSeq: number | undefined, limit: number | undefined) => Promise<Row[]>) {
  const oldestFirst = query.order === 'oldestFirst';
  const segmentsNewestFirst = await readBranchSegmentsNewestFirst(getAgentSqlite(), sessionId, query.start);
...
async function scanEntrySegmentRows(sessionId: string, segment: BranchSegment, query: StorageBranchScan, oldestFirst: boolean, stopSeq: number | undefined, limit: number | undefined): Promise<Row[]> {
  const predicates: string[] = [
    'b.session_id = ?',
    'b.branch_id = ?',
    'b.entry_seq > ?',
...
async function setScalarValueRow(conn: Executor, sessionId: string, namespace: string, key: string, seq: number, storedValue: unknown): Promise<void> {
  exec(conn,
...
async function deleteScalarValueRow(conn: Executor, sessionId: string, namespace: string, key: string): Promise<void> {
  exec(conn,
...
async function appendListValueRow(conn: Executor, sessionId: string, namespace: string, key: string, seq: number, element: unknown): Promise<void> {
  exec(conn,
...
async function deleteListValueRows(conn: Executor, sessionId: string, namespace: string, key: string): Promise<void> {
  exec(conn,
...
async function insertUsageLedgerRow(conn: Executor, sessionId: string, row: Omit<UsageRow, 'seq'> & { seq: number }): Promise<void> {
  exec(conn,
...
async function scanUsageLedgerRows(sessionId: string, query: UsageScan): Promise<UsageRow[]> {
  ...
  const rows = await dbQuery<Row>(`SELECT id, seq, entry_id, adjustment, usage, details FROM agent_usage_ledger WHERE ${filters.join(' AND ')} ${order}${limit}`, params);
  return rows.map((row) => ({
    id: row.id as string,
    seq: Number(row.seq),
...
export class SqliteStorage implements Storage {
  ...
  private async doScanEntries(query: EntryScan): Promise<Entry[]> {
    ...
    const order = query.order === 'desc' ? 'ORDER BY seq DESC' : 'ORDER BY seq ASC';
    const limit = query.limit === undefined ? '' : ` LIMIT ${Math.max(0, query.limit)}`;
    const rows = await dbQuery<Row>(`${ENTRY_SELECT} WHERE ${filters.join(' AND ')} ${order}${limit}`, params);
  ...
  getStats(_context: Context): Promise<import('@earendil-works/pi-agent-core').SessionStats> {
    if (this.state !== 'open') return Promise.reject(new Error('SqliteStorage is closed'));
    return readSessionStats(getAgentSqlite(), this.sessionId);
  }
