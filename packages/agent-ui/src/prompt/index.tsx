...
import usePromptUpload from '../hooks/usePromptUpload';
...
const PromptEditor = forwardRef<PromptEditorRef, PromptEditorProps>(
  (
    ...
    const { run: uploadImage, loading: uploading } = usePromptUpload({
      onSuccess: (result) => {
          ...
          <MentionedImages>
            {mentionedList.map((item, idx) => (
              <MentionedThumb key={item.id}>
                <img
                  src={item.s3Key ? getMediaUrl('thumbnail', item.s3Key) : (item.url ?? '')}
                  alt={item.label}
                  draggable={false}
                />
                <Badge>{idx + 1}</Badge>
