onmessage = function (e) {
  try {
    const { imageData, posX, posY, tolerance, fillColor, canvasConfig } = e.data;
    ...
    function colorsMatch(
      ...
      return Math.abs(r1 - r2) + Math.abs(g1 - g2) + Math.abs(b1 - b2) <= tolerance * 3;
    }

    // 只填充区域
    function floodFill(startX: number, startY: number, fillColor: string, tolerance: number) {
      // 起始点坐标越界检查
      ...
      const fillValue =
        (255 << 24) | // alpha
        (Number(fillColor[2]) << 16) | // blue
        (Number(fillColor[1]) << 8) | // green
        Number(fillColor[0]); // red
      ...
      while (stackPos > 0) {
        ...
        const cr = currentA === 0 ? backgroundRgba[0] : imageData.data[currentIdx];
        const cg = currentA === 0 ? backgroundRgba[1] : imageData.data[currentIdx + 1];
        const cb = currentA === 0 ? backgroundRgba[2] : imageData.data[currentIdx + 2];
        ...
        const checkAdjacent = (ny: number) => {
          ...
          for (let i = left; i <= right; i++) {
            ...
            if (!visited[ny * width + i] && colorsMatch(r, g, b, tr, tg, tb, tolerance)) {
              if (!inRange) {
                // 发现新的需要填充的区域
                stack[stackPos++] = i;
                stack[stackPos++] = ny;
