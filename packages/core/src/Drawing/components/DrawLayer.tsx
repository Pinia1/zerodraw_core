...
interface DrawLayerProps {
  ...
  brushMirrorCanvasRef?: React.RefObject<HTMLCanvasElement | null>;
}

const DrawLayer = React.forwardRef<DrawLayerRef, DrawLayerProps>(
  ({ activeDiagram, brushCanvasRef, brushMirrorCanvasRef }, ref) => {
      ...
      >
        ...
        <KonvaShape
          ...
          sceneFunc={(ctx) => {
            ...
            if (canvas) (ctx as any)._context.drawImage(canvas, 0, 0);
          }}
        />
        {/* libmypaint 笔刷画面：对称镜像 */}
        <KonvaShape
          listening={false}
          sceneFunc={(ctx) => {
            const canvas = brushMirrorCanvasRef?.current;
            if (canvas) (ctx as any)._context.drawImage(canvas, 0, 0);
          }}
        />
